...
- Pick when: the code did not come from `bmad-build`; a PR or branch needs review; a build ran with review skipped or on the quick setting; after material fixes. `bmad-build` does not re-review a finished change: handed a `done` record it treats it as context for new work. After an unattended run that sets `followup_review_recommended`, re-dispatch `bmad-build-auto` on the same `built` plan instead; it goes straight to a fresh review pass.
...
- Writes: a dated `Code Review` block in the joined plan when there is one; otherwise findings stay in the chat.
...
**`bmad-retrospective`** — judges a finished epic as a whole against its requirements.
...
- Pick when: every ticket of the epic is at `built`, `done`, or `dropped`, and especially after unattended runs. An unfinished story forces a rejected verdict.
- Not when: one story or one diff is in question → `bmad-code-review` or `bmad-walkthrough`.
- Writes: `epic-<slug>-retrospective.md` directly in the epic folder; no ticket creation, status change, or epic edit.
